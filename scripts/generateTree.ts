...
async function generateMerkleTree(csvFilePath: string): Promise<void> {
  ...
  for (const row of data) {
    leaf = utils.solidityKeccak256(
      ...
      [row.address, row.tokenId, Number(row.amount)]
    );
    leaves.push(leaf);
  }
  ...
  await new Promise<void>((resolve, reject) => {
    fs.writeFile("merkle_tree2.json", JSON.stringify(addressProofs), (err) => {
      if (err) {
        reject(err);
      } else {
        resolve();
  ...
  await new Promise<void>((resolve, reject) => {
    fs.writeFile("address_data2.json", JSON.stringify(addressData), (err) => {
      if (err) {
        reject(err);
      } else {
        resolve();
